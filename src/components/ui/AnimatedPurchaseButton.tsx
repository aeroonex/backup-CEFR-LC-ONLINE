...
import './AnimatedPurchaseButton.css';
...
const AnimatedPurchaseButton: React.FC<AnimatedPurchaseButtonProps> = ({ onClick, disabled, children }) => {
  ...
  const handleClick = () => {
    if (!disabled) {
      ...
      setTimeout(() => {
        onClick();
        setIsClicked(false); // Reset after action
      }, 600); // Match checkmark animation duration
    }
  };

  return (
    <button
      className={`pay-btn ${isClicked ? 'active' : ''}`}
      onClick={handleClick}
      disabled={disabled}
    >
...
export default AnimatedPurchaseButton;